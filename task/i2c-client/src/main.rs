...
fn run_slave_mode_tests(device: &I2cDevice) {
    ...
    // Test 3: Enable interrupt notification
    uart_send(b"  Test 3: enable_slave_notification... ");
    let notif_result =
        device.enable_slave_notification(notifications::SLAVE_RX_MASK);
    uart_send(if notif_result.is_ok() {
        b"PASS\r\n"
    } else {
        b"FAIL\r\n"
    });
    ringbuf_entry!(Trace::SlaveOpResult(3, notif_result.is_ok()));

    // Test 4: Wait for notification (with timeout) and retrieve message
    uart_send(b"  Test 4: wait for notification (5s timeout)... ");
    sys_set_timer(Some(5000), notifications::TIMER_MASK);

    let mut msg_buf = [0u8; 4];
    let msginfo = sys_recv_open(
        &mut msg_buf,
        notifications::SLAVE_RX_MASK | notifications::TIMER_MASK,
    );

    if msginfo.sender == TaskId::KERNEL {
        if (msginfo.operation & notifications::SLAVE_RX_MASK) != 0 {
            // Notification received! Try to get the message
            match device.get_slave_message() {
                Ok(msg) => {
                    uart_send(b"PASS (received ");
                    print_decimal(msg.data().len() as u8);
                    uart_send(b" bytes)\r\n");
                    ringbuf_entry!(Trace::SlaveOpResult(4, true));
                }
                Err(ResponseCode::NoSlaveMessage) => {
                    uart_send(b"WARN (notification but no message)\r\n");
                    ringbuf_entry!(Trace::SlaveOpResult(4, true));
                }
                Err(_) => {
                    uart_send(b"FAIL (error retrieving message)\r\n");
                    ringbuf_entry!(Trace::SlaveOpResult(4, false));
                }
            }
        } else if (msginfo.operation & notifications::TIMER_MASK) != 0 {
            uart_send(b"TIMEOUT (no data received in 5s)\r\n");
            ringbuf_entry!(Trace::SlaveOpResult(4, true));
        }
    }

    // Test 5: Polling fallback - Get slave message without notification
    uart_send(b"  Test 5: get_slave_message (polling)... ");
    let check_result = device.get_slave_message();
    // Either Ok or NoSlaveMessage is acceptable
    let pass = check_result.is_ok()
        || matches!(check_result, Err(ResponseCode::NoSlaveMessage));
    uart_send(if pass { b"PASS\r\n" } else { b"FAIL\r\n" });
    ringbuf_entry!(Trace::SlaveOpResult(5, pass));
